...
# --- THE DATA IS EMBEDDED IN THE CODE ---
# This eliminates all file-related errors.
# We are using 510 rows of clean data.
csv_data = """ResponseId,EmployeeName,YearsCodePro,DevType,Country,LanguageHaveWorkedWith,ConvertedCompYearly
...
def load_data():
    ...
    try:
        # Read the data directly from the string variable
...
with st.sidebar:
    ...
    if st.button("Load Survey Data"):
        # The function now loads data from the embedded string, not a file
        df = load_data()
        if df is not None:
            st.session_state.df = df
